  ...
  Assets/
    Script/                                     # ALL active code — single source of truth
      ...
      Enemy/                                    # Canonical Assets/Script/Enemy/ — all 6 files
        ...
        EnemyManager.cs                         # ⚠️ SCAFFOLD STUB (unchanged 2026-07-09→07-13) — only `public static EnemyManager Instance` declared; NO lifecycle/aliveCount/events yet (ADR-0002 singleton exception; unused System.Numerics import still present, harmless today — no Vector3 used yet)
        EnemySpawner.cs                         # ✅ Partially implemented (2026-07-13, was a stub 2026-07-09) — OnEnable/OnDisable subscribe `ON_GET_SPAWN_POSITIONS`→`OnDoneLoadRoomGrid` (not `ON_LOAD_MAP` as previously assumed); `SpawnRoomEnemies()` works, compiles clean (`.ConvertTo<T>()` resolves via `Unity.VisualScripting`). ⚠️ `GetRoomSpawnSet()` doesn't null-guard `RoomModel.GetSpawnSet()`'s `null` return (BUG-ES-1); dead `Spawn()` method still present
  ...
  **Setup quan trọng (Inspector):** `RoomGridController` cần wire: `_fastMovement`, `_dungeonRoomSO` (= `Maze_Load_Room.asset`), `_fullDungeonRoomSO` (= `Maze_Storage.asset`), `_listTiles`, `_genmap` (list Tilemap layers). `LevelManager` phải có trong scene với `dungeonRoomSO` = `Maze_Storage.asset`.
  ...
  15. **Enemy spawn system** ⚠️ — GDD `design/gdd/enemy-spawn-system.md` (rewritten 2026-07-13 — actual code is now the primary reference, not an idealized target) + ADR-0002 (EnemyManager singleton). **Prototype** (`Assets/Script/Database-SO/Modal/`): `EntityModel`/`EnemyModal`/`MapModel`/`RoomModel` + `RoomModel.GetSpawnSet()` (two-phase weight-budget, rewritten zero-alloc). **Cập nhật 2026-07-13 — code đã đổi tiếp, không đứng yên từ 07-09:** `MapModel` không còn `idRooms`/`totalWeight` — giờ là `fullRoomList` + `GetRandomRoom()` (rút ngẫu nhiên không lặp lại, tách rời khỏi room identity). Driver **có 2 cái song song**: `EnemySpawner.cs` (event-driven qua `ON_GET_SPAWN_POSITIONS`, không còn rỗng) và nút Editor `LevelManager.SpawnRoomEnemies()` — cả hai đều thiếu null-guard khi `GetSpawnSet()` trả `null` (BUG-ES-1). `RoomGeneraterController` **đã** parse marker `Tile_Spawn_Enemy` (trước ghi nhầm là "chưa có") — 1/13 room JSON có marker, 12 phòng còn lại không có fallback nên sẽ throw. `RoomFile.roomData` (cơ chế map room→preset đã ghi "RESOLVED") **chưa từng được code** — `DungeonRoomSO.RoomFile` vẫn chỉ có `roomName`/`filePath`/`roomType`. `RoomCell` **không có** `GetSpawnPosition()` (ghi chú cũ sai). **SCAFFOLD STUB không đổi:** `Assets/Script/Enemy/EnemyManager.cs` vẫn chỉ khai báo `public static Instance`, chưa có body. **PLANNED (chưa code):** `EnemyManager` lifecycle, emit `ON_ENEMY_DEATH`/`ON_CLEAR_ENEMY`/`ON_ROOM_CLEAR`, fallback vị trí spawn khi phòng không có marker. Phụ thuộc Bug #7/#8 (enemy chưa chết được). GDD's "Future Architecture Direction" đánh giá 3 hướng đi tiếp (giữ code hiện tại / quay lại target id-database cũ / model Candidate Pool + Spawn Chance mới do owner đề xuất) — chưa quyết.
