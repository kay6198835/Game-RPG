# CLAUDE.md
  ...
  > **Last updated:** 2026-09-22 (HEAD `2a83469`, branch `origin/feature/fix-player-control`) —
  ...
  **Effect hierarchy (rebuilt in sprints 13-14):**
  ...
  > 🔴 **Nothing runs at HEAD `2a83469` — the project does not compile (BUG-088).** `EntityMovement.cs:163-164`
  > names two identifiers that do not exist (`Random.range`, `trasnform.postion`). Fix that before
  > reading anything below as a Play Mode statement; everything below is static analysis.
  >
  ...
  **Inspector setup that matters:** `RoomGridController` needs `_dungeonRoomSO`
  ...
  21. **Make Abilities v2 actually work** ⚠️ (raised 2026-09-21, re-scoped twice; latest 2026-09-22
      post-fetch at HEAD `2a83469`) — **fix BUG-088 first: the project does not compile, so none of
      this can be tested.** Then, in this order: **(1) BUG-075** — one word, `OnTriggerEnter2D(Collider2D)`,
      restores projectile damage on its own. **(2) BUG-072** — implement `LightningController`’s overlap
      query, assigning the receiver and invoking the callback once per target; restores summon damage,
      independent of (1). **(3) BUG-089** — a refused `TryCast()` must stop the cast; today it applies
      the effect anyway. **(4) BUG-071 / BUG-074 / BUG-082+BUG-091 / BUG-083.** ✅ No longer on this
      list: BUG-076, BUG-077, BUG-078, BUG-085 — all closed. **The design question that remains is
