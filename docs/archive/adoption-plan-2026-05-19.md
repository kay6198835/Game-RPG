...
> **Reason:** this was a one-time onboarding checklist generated by `/adopt` when the studio
> template was first applied on 2026-05-19. Its purpose was served; the project has since run
> 13 sprints and the artifacts it asks for either exist or are tracked as live gaps elsewhere.
> Keeping it in `docs/` invited it to be read as an outstanding to-do list.
>
> **Archived, not deleted:** it records the project's starting state and which gaps the template
> adoption was meant to close. Nothing below has been edited.
>
> **Superseded by:** `design/gdd/systems-index.md` (what is designed) · `docs/tech-debt-register.md`
> (what is owed) · `production/qa/bugs/` (what is broken) · `memory/project_state.md` (what is
> actually built). Re-run `/adopt` for a current gap analysis rather than reading this.
>
> *Moved here from `docs/adoption-plan-2026-05-19.md` — see `docs/CHANGELOG-DOCS.md`.*
