...
| 17 | **Amendment 3.** `CharacterData` base SO; `PlayerData`/`EntityData` rebased; `CoreBase.Data`; stat/ability bases read it by default | `Character/Base/CharacterData.cs` (+meta), `PlayerData.cs`, `EntityData.cs`, `CoreBase.cs`, `Core.cs`, `EntityCore.cs`, `StatHandlerBase.cs`, `StatHandler.cs`, `EntityStatsHandler.cs`, `EntityEffectStats.cs`, `AbilityHolderBase.cs`, `AbilityHolder.cs`, `EntityAbilityHolder.cs` | ⚠️ fields move to the base with `FormerlySerializedAs` for both sides' old names — no data loss; assets are rewritten on next save | `Player.asset`, `Enemy - Knight.asset`: Stats, AbilityBindings, DefaultWeapon all filled; S |
| 18 | Player abilities no longer need a weapon | `PlayerInputHandle.cs`, `PlayerBasicState.cs` | none | unarmed player: cannot attack, can cast |
| 19 | No weapon, no attack; `EntityAttack` deleted; cadence in `EntityWeaponHolder`; default-weapon equip in `WeaponHolderBase` | `EntityAttack.cs` (+meta, deleted), `WeaponHolderBase.cs`, `EntityWeaponHolder.cs`, `EntityBasicState.cs`, `EntityAttackState.cs`, **prefabs** `Prefab/Enemy/EnemyPrefab.prefab`, `SO/Database/EnemyPrefab.prefab` | ⚠️ component removed by YAML edit; test prefabs (Bat, Crab…) keep a missing-script entry (accepted) | Knight with its weapon removed from data never attacks |
| 20 | Knight weapon content | `Prefab/Weapon/WeaponEntity.prefab`, `SO/Enemy/Weapon/EnemySnS_Stat.asset`, `SO/Enemy/Weapon/Knight_Attack1.asset` (+meta) | missing script replaced by `MeleeWeapon`; stats asset rewritten to current field names | Knight spawns holding the weapon, attacks in range, player loses HP, pause between swings, animator returns to the Knight controller |
