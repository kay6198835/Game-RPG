...
| **Event Manager** (`EventManager.cs`) | Corrected 2026-08-20 — `ON_PLAYER_DEATH` and `ON_ENEMY_DEATH` **now exist** (the enum has 18 values). Only `ON_PLAYER_TAKE_DAMAGE` is still absent, and `.claude/rules/ui-code.md` tells the health bar to bind to it | Character → EventManager |
