...
| `EventID` has 6 values; still no `ON_ENEMY_DEATH` / `ON_ROOM_CLEAR` | **18 values.** Both exist, plus `ON_DONE_SPAWN_ENEMY`, `ON_SPAWN_EXTRA_ENEMY`, `ON_PLAYER_DEATH`, `ON_REALOAD_GAME` and six `ON_*_STATS_*_UI` events. `ON_ROOM_CLEAR` still has no producer |
